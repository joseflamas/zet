...
public class ANTESX extends PApplet {
...
BeatListener bl;
int estereo = 1;
int buffer  = 1024;//2048;//1024
int sampleR = 44100;//48200;//44100
int profBit = 8;//16;//8
...
public void draw()
{
  ...
  if ( beat.isKick() )
    {
      ...
      distMov+=random(2,10);
      distMov = (distMov>20)?20:distMov;
      distMov = (distMov<0)?0:distMov;
    }
  if ( beat.isSnare() )
    {
         anchoL+=random(-15,15);
         anchoL = (anchoL>15)?PApplet.parseInt(random(0,95)):anchoL;
         anchoL = (anchoL<0)?0:anchoL;
         
         anchoR+=random(-15,15);
         anchoR = (anchoR>15)?PApplet.parseInt(random(0,95)):anchoR;
         anchoR = (anchoR<0)?0:anchoR;
         
         sepLR+=random(-50,50);
         sepLR = (anchoR>90)?90:sepLR;
         sepLR = (anchoR<0)?PApplet.parseInt(random(-55,55)):sepLR;
  ...
  {
    normi=i;
    if(i>buffer||i==buffer){ normi = PApplet.parseInt(norm(i,0,buffer)); }
...
class Linea
{
  ...
  public void moverVertical(boolean MOVER, int CUANTO, boolean WIDTH)
  {
    ...
    if(MOVER == true)
    {
     ...
     if(y>distancia||y<0)
     {
       y=PApplet.parseInt(random(100,distancia));
     }
    }
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "ANTESX" };
    if (passedArgs != null) {
